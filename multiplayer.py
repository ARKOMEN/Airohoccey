...
##bure_file="bure.jpeg"
##bure=pygame.image.load(bure_file)
##fedorov_file="fedorov.jpg"
##fedorov=pygame.image.load(fedorov_file)
##Harlamov_file="Harlamov.jpg"
##Harlamov=pygame.image.load(Harlamov_file)
##Harlamov_file="Harlamov.jpg"
##Harlamov=pygame.image.load(Harlamov_file)
##ikushev_file="ikushev.jpg"
##ikushev=pygame.image.load(ikushev_file)
##larionov_file="larionov.jpg"
##larionov=pygame.image.load(larionov_file)
##makarov_file="makarov.jpg"
##makarov=pygame.image.load(makarov_file)
##tretuak_file="tretuak.jpg"
##tretuak=pygame.image.load(tretuak_file)
##helpp_file="help.png"
##helpp1=pygame.image.load(helpp_file)
...
#############################################################################################################################################################################################отбитие от хоккеистов
    if gg == 1:
        if xb - 14 <= hhx1+93 and xb - 14 >= hhx1+88 and yb>=hhy1+30 and yb <= hhy1+127:
            dx=-dx
            dy = -hy1
            tyt1=0
            tyt2=0
            ...
            tyt4=0
            dx=1
        if xb + 14 >= hhx2+34 and xb + 14 <= hhx2+39 and yb>=hhy2+30 and yb <= hhy2+127:
            dy =hy2
            tyt1=0
            tyt2=0
            tyt3=0
...
name=''
def direction1(name):
    global s2,s1, st1,screen
    k1=0#Эта переменная нужна для ввода заглавных букв. Shift здесь не работает
    hor=80
    running=True
    ...
    while running:
        ...
        info_text=font.render("Игрок 1",1, THECOLORS ["red"])
        screen.blit(info_text,[hor,10])
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key==pygame.K_ESCAPE:
                    running = False
                st1=pygame.key.name(event.key)
                if k1==0 and not (st1=="return"):#Фамилия начинается с заглавной буквы
                    st1=st1.title()
                    k1=1
                if st1=="space":
                    st1=" "
                    k1=0#После пробела имя начинается с заглавной буквы
                if st1=="backspace":
                    st1=""
                    a=len(s)-1
                    st1=st1[0:a]
                if not(st1=="return"):#Если не нажата Enter
                    s1=s1+st1
                if st1 =="return":#Если нажата Enter
                    running=False
                st2=pygame.key.name(event.key)
        text565= font.render(s1,0, THECOLORS ["blue"])
        screen.blit(text565, [hor,50] )
        pygame.display.flip()
        screen = pygame.display.set_mode([1071, 672])
def direction2(name):
    global s2,s1, st1,screen#Эта переменная нужна для ввода заглавных букв. Shift здесь не работает
    k2=0
    hor=80
    running=True
    font = pygame.font.SysFont("Arial", 24,False,False)
    ...
    while running:
        ...
        font = pygame.font.SysFont("Arial", 24,False,False)
        myfont = pygame.font.SysFont("Arial", 16,False,False)
        info_text=font.render("Игрок 2",1, THECOLORS ["red"])
        screen.blit(info_text,[hor,10])
        my_text = myfont.render("Press Enter",1, THECOLORS ["black"])
        screen.blit(my_text, [hor+20,120] )
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key==pygame.K_ESCAPE:
                    running = False
                st2=pygame.key.name(event.key)
                if k2==0 and not (st2=="return"):#Фамилия начинается с заглавной буквы
                    st2=st2.title()
                    k2=1
                if st2=="space":
                    st2=" "
                    k2=0#После пробела имя начинается с заглавной буквы
                if st2=="backspace":
                    st2=""
                    a=len(s)-1
                    st1=st1[0:a]
                if not(st2=="return"):#Если не нажата Enter
                    s2=s2+st2
                if st2 =="return":#Если нажата Enter
                    running=False
        text656= font.render(s2,0, THECOLORS ["blue"])
        screen.blit(text656, [hor,50] )
        pygame.display.flip()
        screen = pygame.display.set_mode([1071, 672])
    return(name)
...
def multiplayer():
    global s1,s2,down1,screen,my_down2,gg,xd,yd,pas ,i,hokkeyblue, my_downn,xmn,ymn,io, pi,dy,dx,do,re,mi,fa,sole,li,ci,cibemole,time,timer,timet,gole1,gole2,xb,yb,hx1,hy1,hx2,hy2,hhx1,hhy1,hhx2,hhy2,otbitie1,otbitie2,iu1,iu2,tyt1,tyt2,tyt3,tyt4
    my_down2 = True
    direction1(name)
    direction2(name)
    while my_down2:
        ...
        text565= font.render(s1,0, THECOLORS ["blue"])
        screen.blit(text565, [10,100] )
        font = pygame.font.SysFont("Arial", 60,False,False)
        text656= font.render(s2,0, THECOLORS ["blue"])
        screen.blit(text656, [640,100] )
